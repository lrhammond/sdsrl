...
schema_x_pos(Obj, New):t <- nb8(Obj,Nb8):t, nothing(Nb8):t ~= yes, nb1(Obj,Nb1):t, colour(Nb1):t ~= wall, nb6(Obj,Nb6):t, nothing(Nb6):t ~= yes, action(l), x_pos(Obj):t ~= Curr, New is Curr - 1.
x_pos(Obj):t+1 ~ val(New) <- schema_x_pos(Obj, New):t.
x_pos(Obj):t+1 ~ val(Curr) <- x_pos(Obj):t ~= Curr.
schema_y_pos(Obj, New):t <- nb5(Obj,Nb5):t, colour(Nb5):t ~= goal, y_pos(Obj):t ~= Curr, New is Curr - 1.
