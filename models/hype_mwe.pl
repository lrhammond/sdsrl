...
% r1:t ~ val(10) <- is_object(Obj), nb4(Obj,Nb4):t, type(Nb4):t ~= goal, action(r).
% r1:t ~ val(10) <- true.
% r2:t ~ val(-10) <- is_object(Obj), nb4(Obj,Nb4):t, type(Nb4):t ~= goal, action(l).
% r2:t ~ val(10) <- true.
% r3:t ~ val(-10) <- is_object(Obj), nb6(Obj,Nb6):t, type(Nb6):t ~= hole, action(d).
% r3:t ~ val(10) <- true.
% % reward:t ~ val(-1) <- constraints:t.
% reward:t ~ val(R) <- r1:t ~= R1, r2:t ~= R2, r3:t ~= R3, R is R1 + R2 + R3.

is_object(Obj) <- member(Obj, [obj0,obj1,obj2,obj3,obj4,obj5,obj6,obj7,obj8,obj9,obj10,obj11,obj12,obj13,obj14,obj15,obj16,obj17]).


reward:t ~ val(-100) <- is_object(Obj), map(1, 1, Obj):t.
reward:t ~ val(100) <- is_object(Obj), map(3, 1, Obj):t.
reward:t ~ val(-1) <- is_object(Obj), \+((map(1, 1, Obj):t)), \+((map(3, 1, Obj):t)).
