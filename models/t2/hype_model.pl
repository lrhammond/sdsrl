...
schema_y_pos(Obj, New):t <- nb4(Obj):t ~= Nb4, nothing(Nb4):t ~= yes, colour(Obj):t ~= agent, action(d), y_pos(Obj):t ~= Curr, New is Curr - 1.
...
reward:t ~ val(-10) <- constraints:t, changed(Obj), nb6(Obj):t ~= Nb6, colour(Nb6):t ~= hole, action(d).
reward:t ~ val(10) <- constraints:t, changed(Obj), nb8(Obj):t ~= Nb8, colour(Nb8):t ~= hole, action(r).
