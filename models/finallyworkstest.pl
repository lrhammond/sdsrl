...
map(Obj, NX, NY):t+1 <- new_x_pos(Obj, NX):t, new_y_pos(Obj, NY):t.




schema_x_pos(Obj, New, Nb4):t <- colour(Obj):t ~= Agent, action(r), x_pos(Obj):t ~= X, y_pos(Obj):t ~= Y, New is X + 1, Nb4 = no_object.

schema_x_pos(Obj, New, Nb4):t <- colour(Obj):t ~= Agent, action(l), x_pos(Obj):t ~= Curr, New is Curr - 1.

no_schema_x_pos(Obj, Curr, Nb4):t <- \+schema_x_pos(Obj, _, Nb4):t, x_pos(Obj):t ~= Curr.


x_pos(Obj):t+1 ~ val(New) <- x_pos(Obj):t ~= X, y_pos(Obj):t ~= Y, nb4pred(X, Y, Nb4):t, schema_x_pos(Obj, New, Nb4):t.
x_pos(Obj):t+1 ~ val(New) <- x_pos(Obj):t ~= X, y_pos(Obj):t ~= Y, nb4pred(X, Y, Nb4):t, no_schema_x_pos(Obj, New, Nb4):t.
...
% r(Xobj, Y, Type, R) <- R = 10000, Xobj > 3, Type=agent.

% r(X, Y, Type, R) <- R = 10000, nb4pred(X, Y, Nb4):t, Type=agent.
...
r(Xobj, Y, Type, R) <- R = -100, Xobj=1, Type=agent.
% r(Xobj, Yobj, Type, R):t <- R = -10, action(l), Xobj=2, Yobj=3, Type=agent.
r(Xobj, Y, Type, R) <- R = 1000, Xobj=3, Type=agent.


schema_reward(Obj):t ~ val(R) <- x_pos(Obj):t ~= X, y_pos(Obj):t ~= Y, colour(Obj):t ~= Type, r(X, Y, Type, R).
schema_reward(Obj):t ~ val(-1) <- x_pos(Obj):t ~= X, y_pos(Obj):t ~= Y, colour(Obj):t ~= Type, \+r(X, Y, Type, _).
