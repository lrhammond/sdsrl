...
% Attribute Schemas
schema_x_pos(Obj, New):t <- nb6(Obj,Nb6):t, colour(Nb6):t ~= hole, action(r), x_pos(Obj):t ~= Curr, New is Curr + 1.
schema_x_pos(Obj, New):t <- nb4(Obj,Nb4):t, nothing(Nb4):t ~= yes, nb1(Obj,Nb1):t, colour(Nb1):t ~= wall, nb6(Obj,Nb6):t, nothing(Nb6):t ~= yes, action(r), x_pos(Obj):t ~= Curr, New is Curr + 1.
schema_x_pos(Obj, New):t <- nb4(Obj,Nb4):t, nothing(Nb4):t ~= yes, nb5(Obj,Nb5):t, nothing(Nb5):t ~= yes, nb7(Obj,Nb7):t, colour(Nb7):t ~= wall, colour(Obj):t ~= agent, nb3(Obj,Nb3):t, colour(Nb3):t ~= wall, nb8(Obj,Nb8):t, colour(Nb8):t ~= wall, action(r), x_pos(Obj):t ~= Curr, New is Curr + 1.
schema_x_pos(Obj, New):t <- nb5(Obj,Nb5):t, colour(Nb5):t ~= goal, action(l), x_pos(Obj):t ~= Curr, New is Curr - 1.
schema_x_pos(Obj, New):t <- nb6(Obj,Nb6):t, colour(Nb6):t ~= wall, nb7(Obj,Nb7):t, nothing(Nb7):t ~= yes, nb4(Obj,Nb4):t, colour(Nb4):t ~= wall, nb3(Obj,Nb3):t, nothing(Nb3):t ~= no, action(l), x_pos(Obj):t ~= Curr, New is Curr - 1.
schema_x_pos(Obj, New):t <- nb5(Obj,Nb5):t, colour(Nb5):t ~= wall, nb3(Obj,Nb3):t, colour(Nb3):t ~= wall, nb7(Obj,Nb7):t, nothing(Nb7):t ~= yes, nb6(Obj,Nb6):t, nothing(Nb6):t ~= yes, action(l), x_pos(Obj):t ~= Curr, New is Curr - 1.
x_pos(Obj):t+1 ~ val(New) <- schema_x_pos(Obj, New):t.
x_pos(Obj):t+1 ~ val(Curr) <- x_pos(Obj):t ~= Curr.
schema_y_pos(Obj, New):t <- nb2(Obj,Nb2):t, colour(Nb2):t ~= wall, nb1(Obj,Nb1):t, colour(Nb1):t ~= wall, nb6(Obj,Nb6):t, nothing(Nb6):t ~= yes, action(d), y_pos(Obj):t ~= Curr, New is Curr - 1.
