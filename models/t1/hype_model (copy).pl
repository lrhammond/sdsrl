...
schema_y_pos(Obj, New):t <- nb4(Obj,Nb4):t, nothing(Nb4):t ~= yes, colour(Obj):t ~= agent, action(d), y_pos(Obj):t ~= Curr, New is Curr - 1.
...
% Reward Schemas
R1:t ~ val(-1) <- constraints:t, changed(obj19), nb6(obj19,Nb6):t, colour(Nb6):t ~= hole, action(r).
R1:t ~ val(0) <- true.
R2:t ~ val(-10) <- constraints:t, nb6(obj19,Nb6):t, colour(Nb6):t ~= hole, action(d).
R1:t ~ val(0) <- true.
% R2:t ~ val(X) <- X is max(0, abs(R1:t)).
R3:t ~ val(10) <- constraints:t, nb8(obj19,Nb8):t, colour(Nb8):t ~= hole, action(r).
R3:t ~ val(0) <- true.
% R2:t ~ val(X) <- X is max(0, abs(R1:t)).
R4:t ~ val(-10) <- constraints:t, nb8(obj19,Nb8):t, colour(Nb8):t ~= hole, action(l).
R4:t ~ val(0) <- true.
R5:t ~ val(-1) <- constraints:t, changed(Obj), nb2(Obj,Nb2):t, colour(Nb2):t ~= wall, colour(Obj):t ~= agent.
R5:t ~ val(0) <- true.
R6:t ~ val(-1) <- constraints:t, changed(Obj), nb8(Obj,Nb8):t, nothing(Nb8):t ~= yes, colour(Obj):t ~= agent.
R6:t ~ val(0) <- true.
% R:t ~ val(0) <- true.

reward:t ~ val(R) <- R is R1:t + R2:t + R3:t + R4:t + R5:t + R6:t.
