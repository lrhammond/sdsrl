...
% Neighbours
nb1(Obj):t ~ val(Nb) <- x_pos(Obj):t ~= X, y_pos(Obj):t ~= Y, NbX is X - 1, NbY is Y + 1, map(NbX, NbY, Nb):t.
nb2(Obj):t ~ val(Nb) <- x_pos(Obj):t ~= X, y_pos(Obj):t ~= Y, NbX is X,     NbY is Y + 1, map(NbX, NbY, Nb):t.
nb3(Obj):t ~ val(Nb) <- x_pos(Obj):t ~= X, y_pos(Obj):t ~= Y, NbX is X + 1, NbY is Y + 1, map(NbX, NbY, Nb):t.
nb4(Obj):t ~ val(Nb) <- x_pos(Obj):t ~= X, y_pos(Obj):t ~= Y, NbX is X + 1, NbY is Y    , map(NbX, NbY, Nb):t.
nb5(Obj):t ~ val(Nb) <- x_pos(Obj):t ~= X, y_pos(Obj):t ~= Y, NbX is X + 1, NbY is Y - 1, map(NbX, NbY, Nb):t.
nb6(Obj):t ~ val(Nb) <- x_pos(Obj):t ~= X, y_pos(Obj):t ~= Y, NbX is X,     NbY is Y - 1, map(NbX, NbY, Nb):t.
nb7(Obj):t ~ val(Nb) <- x_pos(Obj):t ~= X, y_pos(Obj):t ~= Y, NbX is X - 1, NbY is Y - 1, map(NbX, NbY, Nb):t.
nb8(Obj):t ~ val(Nb) <- x_pos(Obj):t ~= X, y_pos(Obj):t ~= Y, NbX is X - 1, NbY is Y    , map(NbX, NbY, Nb):t.
...
changed(Obj) <- member(Obj, [obj19,obj0]).

% Constraints
constraints:t <- true.

% Attribute Schemas
schema_x_pos(Obj, New):t <- nb4(Obj):t ~= Nb4, nothing(Nb4):t ~= yes, colour(Obj):t ~= agent, action(r), x_pos(Obj):t ~= Curr, New is Curr + 1.
...
% Reward Schemas
% reward:t ~ val(-1) <- constraints:t, changed(obj19), colour(nb6(obj19):t):t ~= hole, action(r).
% q:t <- constraints:t, changed(obj19), colour(nb6(obj19):t):t ~= hole, action(r).
% % R1:t ~ val(0) <- true.
% reward:t ~ val(-10) <- constraints:t, colour(nb6(obj19):t):t ~= hole, action(d).
% q:t <- constraints:t, colour(nb6(obj19):t):t ~= hole, action(d).
% % R2:t ~ val(0) <- true.
% % R2:t ~ val(X) <- X is max(0, abs(R1:t)).
% reward:t ~ val(10) <- constraints:t, colour(nb8(obj19):t):t ~= hole, action(r).
% q:t <- constraints:t, colour(nb8(obj19):t):t ~= hole, action(r).
% % R3:t ~ val(0) <- true.
% % R2:t ~ val(X) <- X is max(0, abs(R1:t)).
% reward:t ~ val(-10) <- constraints:t, colour(nb8(obj19):t):t ~= hole, action(l).
% q:t <- constraints:t, colour(nb8(obj19):t):t ~= hole, action(l).
% % R4:t ~ val(0) <- true.
% reward:t ~ val(-1) <- constraints:t, changed(obj19), colour( nb2(obj19):t):t ~= wall, colour(obj19):t ~= agent.
% q:t <- constraints:t, changed(obj19), colour( nb2(obj19):t):t ~= wall, colour(obj19):t ~= agent.
% % R5:t ~ val(0) <- true.
% reward:t ~ val(-1) <- constraints:t, changed(obj19), nothing(nb8(obj19):t):t ~= yes, colour(obj19):t ~= agent.
% q:t <- constraints:t, changed(obj19), nothing(nb8(obj19):t):t ~= yes, colour(obj19):t ~= agent.
% % R6:t ~ val(0) <- true.
% % R:t ~ val(0) <- true.
% reward:t ~ val(0) <- \q:t.
% % reward:t ~ val(R) <- R is 0.




reward:t ~ val(-1) <- constraints:t, changed(Obj), nb6(Obj):t ~= Nb6, colour(Nb6):t ~= hole, action(r).
q:t ~ val(-1) <- constraints:t, changed(Obj), nb6(Obj):t ~= Nb6, colour(Nb6):t ~= hole, action(r).
% R1:t ~ val(0) <- true.
reward:t ~ val(-10) <- constraints:t, nb6(Obj):t ~= Nb6, colour(Nb6):t ~= hole, action(d).
q:t ~ val(-10) <- constraints:t, nb6(Obj):t ~= Nb6, colour(Nb6):t ~= hole, action(d).
% R2:t ~ val(0) <- true.
% R2:t ~ val(X) <- X is max(0, abs(R1:t)).
reward:t ~ val(10) <- constraints:t, nb8(Obj):t ~= Nb8, colour(Nb8):t ~= hole, action(r).
q:t ~ val(10) <- constraints:t, nb8(Obj):t ~= Nb8, colour(Nb8):t ~= hole, action(r).
% R3:t ~ val(0) <- true.
% R2:t ~ val(X) <- X is max(0, abs(R1:t)).
reward:t ~ val(-10) <- constraints:t, nb8(Obj):t ~= Nb6, colour(Nb8):t ~= hole, action(l).
q:t ~ val(-10) <- constraints:t, nb8(Obj):t ~= Nb6, colour(Nb8):t ~= hole, action(l).
% R4:t ~ val(0) <- true.
reward:t ~ val(-1) <- constraints:t, changed(Obj), nb2(Obj):t ~= Nb2, colour(Nb2):t ~= wall, colour(Obj):t ~= agent.
q:t ~ val(-1) <- constraints:t, changed(Obj), nb2(Obj):t ~= Nb2, colour(Nb2):t ~= wall, colour(Obj):t ~= agent.
% R5:t ~ val(0) <- true.
reward:t ~ val(-1) <- constraints:t, changed(Obj), nb8(Obj):t ~= Nb8, nothing(Nb8):t ~= yes, colour(Obj):t ~= agent.
q:t ~ val(-1) <- constraints:t, changed(Obj), nb8(Obj):t ~= Nb8, nothing(Nb8):t ~= yes, colour(Obj):t ~= agent.
