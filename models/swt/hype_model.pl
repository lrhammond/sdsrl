...
% Attribute Schemas
x_pos_right_0(Obj):t ~ val(1.0) <- nb4(Obj,Nb4):t, nothing(Nb4):t ~= yes, colour(Obj):t ~= agent, action(r).
x_pos_right_0(Obj):t ~ val(0) <- true.
x_pos_left_0(Obj):t ~ val(0.8) <- colour(Obj):t ~= agent, action(l).
x_pos_left_0(Obj):t ~ val(0) <- true.
x_pos(Obj):t+1 ~ val(Curr) <- x_pos(Obj):t ~= Curr, x_pos_right_0(Obj):t ~= 0, x_pos_left_0(Obj):t ~= 0.
x_pos(Obj):t+1 ~ finite([P_right : Right, P_left : Left, P_curr : Curr]) <- x_pos(Obj):t ~= Curr, Right is Curr + 1, Left is Curr - 1, x_pos_right_0(Obj):t ~= X_pos_right_0, x_pos_left_0(Obj):t ~= X_pos_left_0, P_curr is (1 - X_pos_right_0) * (1 - X_pos_left_0), P_right is (X_pos_right_0) * (1 - P_curr) / (X_pos_right_0 + X_pos_left_0), P_left is (X_pos_left_0) * (1 - P_curr) / (X_pos_right_0 + X_pos_left_0).
y_pos_below_0(Obj):t ~ val(1.0) <- nb7(Obj,Nb7):t, colour(Nb7):t ~= wall, nb8(Obj,Nb8):t, nothing(Nb8):t ~= yes, nb4(Obj,Nb4):t, nothing(Nb4):t ~= yes, nb6(Obj,Nb6):t, nothing(Nb6):t ~= yes.
y_pos_below_0(Obj):t ~ val(0) <- true.
y_pos_below_1(Obj):t ~ val(0.5) <- nb5(Obj,Nb5):t, nothing(Nb5):t ~= yes, nb7(Obj,Nb7):t, nothing(Nb7):t ~= yes, colour(Obj):t ~= agent, nb6(Obj,Nb6):t, nothing(Nb6):t ~= yes.
y_pos_below_1(Obj):t ~ val(0) <- true.
y_pos_above_0(Obj):t ~ val(0.5) <- colour(Obj):t ~= agent, action(u).
y_pos_above_0(Obj):t ~ val(0) <- true.
y_pos(Obj):t+1 ~ val(Curr) <- y_pos(Obj):t ~= Curr, y_pos_below_0(Obj):t ~= 0, y_pos_below_1(Obj):t ~= 0, y_pos_above_0(Obj):t ~= 0.
y_pos(Obj):t+1 ~ finite([P_below : Below, P_above : Above, P_curr : Curr]) <- y_pos(Obj):t ~= Curr, Below is Curr - 1, Above is Curr + 1, y_pos_below_0(Obj):t ~= Y_pos_below_0, y_pos_below_1(Obj):t ~= Y_pos_below_1, y_pos_above_0(Obj):t ~= Y_pos_above_0, P_curr is (1 - Y_pos_below_0) * (1 - Y_pos_below_1) * (1 - Y_pos_above_0), P_below is (Y_pos_below_0 + Y_pos_below_1) * (1 - P_curr) / (Y_pos_below_0 + Y_pos_below_1 + Y_pos_above_0), P_above is (Y_pos_above_0) * (1 - P_curr) / (Y_pos_below_0 + Y_pos_below_1 + Y_pos_above_0).
...
% Reward Schemas
reward:t ~ val(10) <- constraints:t, is_object(Obj), nb2(Obj,Nb2):t, colour(Nb2):t ~= goal, action(u).
...
observation(x_pos(obj51)) ~= 2, observation(y_pos(obj51)) ~= 5, observation(x_size(obj51)) ~= 1, observation(y_size(obj51)) ~= 1, observation(colour(obj51)) ~= agent, observation(shape(obj51)) ~= square, observation(nothing(obj51)) ~= no,
