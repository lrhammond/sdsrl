...
:- set_options(default), set_inference(backward(lazy)).

% to represent the belief at time t of maze(X,Y):t use belief(maze(X,Y)) in the body (internally: belief(A):-ground(A),!.)
...
% Alternative defintion of the maze:
